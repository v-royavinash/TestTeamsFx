...
class Dashboard extends React.PureComponent<IDashboardProps, IDashboardState> {
    ...
    editButton = (cell: any, gridRow: any, rowIndex: any, formatExtraData: any) => {
        if (gridRow.status === constants.closed) {
            return (
                <span />
            );
        }
    ...
    statusIcon = (cell: any, row: any, rowIndex: any, formatExtraData: any) => {
        if (row.status === constants.closed) {
            return (
                <img src={require("../assets/Images/ClosedIcon.svg").default} alt="Closed Icon" width="31" height="31" />
            );
        }
        if (row.status === constants.active) {
            return (
                <img src={require("../assets/Images/ActiveIcon.svg").default} alt="Active Icon" width="31" height="31" />
            );
        }
        if (row.status === constants.planning) {
            return (
                <img src={require("../assets/Images/PlanningIcon.svg").default} alt="Planning Icon" width="31" height="31" />
            );
        }
    };
    ...
    searchDashboard = (searchText: any) => {
        ...
        let filteredAllIncidents = allIncidents.filter((incident: any) => {
            return ((incident["incidentName"] && incident["incidentName"].toLowerCase().indexOf(searchKeyword) > -1) ||
                (incident["incidentId"] && incident["incidentId"].toLowerCase().indexOf(searchKeyword) > -1) ||
                                ...
                                >
                                    ...
                                    >
                                        <BootstrapTable
                                            ...
                                            bordered={false}
                                            keyField="incidentId"
                                            columns={dashboardHeader.slice(0, 6)}
