...
import { PeoplePicker, LocalizationHelper } from '@microsoft/mgt-react';
...
import {
    ...
    IInputValidationStates, IOpenShift, ITeamChannel, ITeamCreatedResponse,
    RoleAssignments, ShiftGroupInfo
...
const getInputRegexValildationInitialState = (): IInputRegexValidationStates => {
    return {
        incidentNameHasError: false,
        incidentLocationHasError: false
    };
...
class IncidentDetails extends React.PureComponent<IIncidentDetailsProps, IIncidentDetailsState> {
    constructor(props: IIncidentDetailsProps) {
        ...
        this.state = {
            ...
            inputRegexValidation: getInputRegexValildationInitialState(),
        ...
        try {
            ...
            await Promise.all([statusOptionsPromise, typeOptionsPromise, roleOptionsPromise])
                .then(([statusOptions, typeOptions, roleOptions]) => {
                    ...
                    optionsArr.typeOptions = typeOptions;
                    optionsArr.roleOptions = roleOptions;
                    optionsArr.roleOptions.push("Other");
                    this.setState({
                    ...
                    this.props.showMessageBar(this.props.localeStrings.genericErrorMessage, constants.messageBarType.error);
                    this.setState({
                        showLoader: false,
                        formOpacity: 1
                    })
    ...
    getIncident = async (incName: string): Promise<boolean> => {
        let incNameStr = incName.replace(/'/g, "''");

        // create graph endpoint for querying Incident Transaction list
        this.graphEndpoint = `${graphConfig.spSiteGraphEndpoint}${this.props.siteId}${graphConfig.listsGraphEndpoint}/${siteConfig.incidentsList}/items?$expand=fields&$filter=fields/Title eq '${incNameStr}'`;

        try {
            const incDetails = await this.dataService.getExistingIncident(this.graphEndpoint, this.props.graph);
            ...
            console.error(
                constants.errorLogPrefix + "CreateIncident_GetIncident \n",
                JSON.stringify(error)
            );
            return false;
        }
    ...
    private addNewRole = async () => {
        ...
        try {
            ...
            if (addedRole) {
                ...
                // reorder the roles array to push "Other" at last
                this.reorderArray(arr["roleOptions"],
                    arr["roleOptions"].indexOf("Other"),
                    (arr["roleOptions"].length - 1));
    ...
    // Reorders Role assignment array to move "Other" option at last
    private reorderArray = (rolesArr: [], from: number, to: number): any => {

        // Make sure a valid array is provided
        if (Object.prototype.toString.call(rolesArr) !== '[object Array]') {
            this.props.showMessageBar(this.props.localeStrings.genericErrorMessage, constants.messageBarType.error);
        }

        // Delete the item from it's current position
        const item = rolesArr.splice(from, 1);
        ...
        return [...rolesArr.splice(to, 0, item[0])];
    };
    ...
    private checkAddRoleBtnState = () => {
        if (this.state.createIncDetailsItem.selectedRole !== "" &&
            this.state.createIncDetailsItem.assignedUser && this.state.createIncDetailsItem.assignedUser.length > 0) {
            this.setState({
    ...
    private createNewIncident = async () => {
        ...
        if (!this.requiredFieldValidation(incidentInfo)) {
            ...
            try {
                ...
                if (!this.regexValidation(incidentInfo)) {
                    ...
                    if (await this.getIncident(incidentInfo.incidentName)) {
                        this.setState({
                            showLoader: false,
                            formOpacity: 1
                        });
    ...
    private regexValidation = (incidentInfo: CreateIncidentEntity): boolean => {
        let inputRegexValidationObj = getInputRegexValildationInitialState();
        ...
        if (inputRegexValidationObj.incidentLocationHasError || inputRegexValidationObj.incidentNameHasError) {
            this.props.showMessageBar(this.props.localeStrings.regexErrorMessage, constants.messageBarType.error);
            this.setState({
                inputRegexValidation: inputRegexValidationObj,
                showLoader: false,
                formOpacity: 1
            });
            regexvalidationSuccess = false;
        }
        return regexvalidationSuccess;
    }
    ...
    private createTeamGroup = async (incId: string): Promise<any> => {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                // add current user as a member if already not present so that we can perform
                // sharepoint site related operations on associated team site
                if (membersArr.indexOf(graphConfig.usersGraphEndpoint + this.props.currentUserId) === -1) {
                    membersArr.push(graphConfig.usersGraphEndpoint + this.props.currentUserId)
                }

                // create object to create teams group
                let incidentobj = {
                    displayName: `${constants.teamEOCPrefix}-${incId}-${incDetails.location.trim()}-${incDetails.incidentType}-${incDetails.startDateTime}`,
                    mailNickname: `${constants.teamEOCPrefix}_${incId}_${incDetails.location.replace(/[^a-zA-Z0-9_-]/g, '_')}_${incDetails.incidentType.replace(/[^a-zA-Z0-9_-]/g, '_')}`,
                    ...
                    "owners@odata.bind": [graphConfig.usersGraphEndpoint + incDetails.incidentCommander.userId]
                }
                                ...
                                <div className="container">
                                        ...
                                        <Col md={4} sm={8} xs={12}>
                                            ...
                                            <div className="incident-grid-item">
                                                <FormDropdown
                                                    ...
                                                    className={this.state.createIncDetailsItem && this.state.createIncDetailsItem.incidentType ? "incident-details-dropdown" : "dropdown-placeholder"}
                                                />
                                    ...
                                    <Row xs={1} sm={1} md={2}>
                                        <Col md={6} sm={8} xs={12}>
                                            ...
                                            {this.state.createIncDetailsItem.selectedRole === 'Other' ?
                                                <>
                                                    <div className="incident-grid-item">
                                                        <FormInput
